...
/**
 请求同步JS文心一言数据(答:文字)｜Request to synchronize JS ERNIE Bot data (Answer: Text)
...
/**
 请求同步JS文心一言数据(答:音频)｜Request to synchronize JS ERNIE Bot data (Answer: Audio)
 @param     answer      回答内容｜Answer content
 @param     callback    结果回调｜Result callback
 
 @note 仅部分设备支持｜Only supported on some devices
 */
+ (void)requestSyncJsERNIE_BoWithAnswerAudio:(NSURL * _Nonnull)answer callback:(void(^)(NSError * _Nullable error))callback;
